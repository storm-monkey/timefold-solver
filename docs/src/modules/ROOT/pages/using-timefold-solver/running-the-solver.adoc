...
Call `Solver.solveAndGetResult(Solution)` to receive both the best solution and metadata about the run,
such as the solving duration, score calculation count, move evaluation speed,
problem size statistics and the termination types that stopped the run.
...
regardless of the xref:using-timefold-solver/running-the-solver.adoc#multithreadedIncrementalSolving[`moveThreadCount`] of the solvers.
...
Call `SolverJob.getFinalBestSolutionResult()` to retrieve the same run metadata that is available from a blocking `Solver`.
If you use a final best solution event consumer,
the `FinalBestSolutionEvent.solverResult()` method exposes that metadata in the consumer thread.
...
Additionally,
an improved solution consumer capable of throttling events is available in the xref:using-timefold-solver/running-the-solver.adoc#throttlingBestSolutionEvents[Enterprise Edition] of the Timefold Solver.
...
may use the xref:using-timefold-solver/running-the-solver.adoc#throttlingBestSolutionEvents[throttling feature]
